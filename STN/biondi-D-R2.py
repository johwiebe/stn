...
# -*- coding: utf-8 -*-
"""
Vary D parameters for Reaction 3 on Reactor 2
"""
    ...
    for d2 in D2range:
        for d3 in D2range:
            if d3 > d2 and d2 > d1:
                ...
                pref = str(d1) + "-" + str(d2) + "-" + str(d3)
                stn.build(TIMEs,TIMEp)
                stn.solve('cplex',prefix=pref)
                stn.gantt(prefix=pref)
                stn.trace(prefix=pref)
                stn.trace_planning(prefix=pref)
                stn.eval()
