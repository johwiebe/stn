...
#        self.p[task] = max(self.p[task],dur)
    ...
    def build(self, TIMEs, TIMEp):
        ...
        def scheduling_block(b):
            ...
            b.l = Var([1, 2, 3, 4], self.units, b.TIME, self.tasks, self.opmodes, b.TIME,
                      domain=NonNegativeReals)
            b.u = Var([1, 2, 3, 4], self.units, b.TIME, self.tasks, self.opmodes, b.TIME,
                      domain=NonNegativeReals)

            # Variables for residual life affine decision rule
            b.Rc = Var(self.units, b.TIME, self.tasks, self.opmodes, b.TIME,
                       domain=NonNegativeReals)
            b.R0 = Var(self.units, b.TIME, domain=NonNegativeReals)
...
#                                    b.cons.add(sum([b.W[i1,j,k,t1] for k in self.O[j]]) + sum([b.W[i2,j,k,t2] for k in self.O[k]]) <= 1)
            ...
            for j in self.units:
                ...
                for t in b.TIME:
                    ...
                    b.cons.add(self.Rmax[j]*b.M[j,t] <= b.R[j,t])
                    ...
                    for i in self.I[j]:
                        for k in self.O[j]:
                            for tprime in b.TIME[b.TIME <= t]:
                                lhs += self.D[i,j,k] * ((1 + self.eps)
                                                        * b.u[1,j,t,i,k,tprime]
                                                        - (1 - self.eps)
                                                        * b.l[1,j,t,i,k,tprime])
                                b.cons.add(b.u[1,j,t,i,k,tprime] -
                                           b.l[1,j,t,i,k,tprime] >= -
                                           b.Rc[j,t,i,k,tprime])
                    rhs = b.R0[j,t]
                    b.cons.add(lhs <= rhs)
                    ...
                    for i in self.I[j]:
                        for k in self.O[j]:
                            for tprime in b.TIME[b.TIME <= t]:
                                lhs += self.D[i,j,k] * ((1 + self.eps)
                                                        * b.u[2,j,t,i,k,tprime]
                                                        - (1 - self.eps)
                                                        * b.l[2,j,t,i,k,tprime])
                                b.cons.add(b.u[2,j,t,i,k,tprime] -
                                           b.l[2,j,t,i,k,tprime] >=
                                           b.Rc[j,t,i,k,tprime])
                    rhs = -b.R0[j,t] + self.Rmax[j]*(1 - b.M[j,t])
                    #rhs = -b.R0[j,t] + self.Rmax[j]
                    b.cons.add(lhs <= rhs)
                    ...
                    for i in self.I[j]:
                        for k in self.O[j]:
                            for tprime in b.TIME[b.TIME <= t]:
                                lhs += self.D[i,j,k] * ((1 + self.eps)
                                                        * b.u[3,j,t,i,k,tprime]
                                                        - (1 - self.eps)
                                                        * b.l[3,j,t,i,k,tprime])
                                ...
                                if (t == b.TIME[0]):
                                    ...
                                    R0Last = b.R0[j,t-self.dTs]
                                    RcLast = b.Rc[j,t-self.dTs,i,k,tprime]

                                if (tprime == t):
                                    b.cons.add(b.u[3,j,t,i,k,tprime]
                                               - b.l[3,j,t,i,k,tprime]
                                               >=
                                               - b.Rc[j,t,i,k,tprime]
                                               + b.W[i,j,k,t])
                    ...
                    for i in self.I[j]:
                        for k in self.O[j]:
                            for tprime in b.TIME[b.TIME <= t]:
                                lhs += self.D[i,j,k] * ((1 + self.eps)
                                                        * b.u[4,j,t,i,k,tprime]
                                                        - (1 - self.eps)
                                                        * b.l[4,j,t,i,k,tprime])
                                ...
                                if (tprime == t):
                                    b.cons.add(b.u[4,j,t,i,k,tprime]
                                               - b.l[4,j,t,i,k,tprime]
                                               >=
                                               + b.Rc[j,t,i,k,tprime]
                                               - b.W[i,j,k,t])
        ...
        def planning_block(b):
            ...
            b.l = Var([1, 2, 3], self.units, b.TIME, self.tasks, self.opmodes, b.TIME,
                      domain=NonNegativeReals)
            b.u = Var([1, 2, 3], self.units, b.TIME, self.tasks, self.opmodes, b.TIME,
                      domain=NonNegativeReals)
            b.Rc = Var(self.units, b.TIME, self.tasks, self.opmodes, b.TIME,
                       domain=NonNegativeReals)
            b.R0 = Var(self.units, b.TIME, domain=NonNegativeReals)
            ...
            for j in self.units:
                for t in b.TIME:
                    ...
                    #            if (t == b.TIME[0]):
                    #                R0Last = m.Rtransfer[j]
                    #                RcLast = 0
                    #            else:
                    #                R0Last = b.R0[j,t-self.dTp]
                    #                RcLast = b.Rc[j,t-self.dTp,i,k,tprime]
        ...
        m.usb = Var(self.tasks, self.units, self.opmodes, m.sb.TIME,
                    domain=NonNegativeReals)
        m.lsb = Var(self.tasks, self.units, self.opmodes, m.sb.TIME,
                    domain=NonNegativeReals)
        m.upb = Var(self.tasks, self.units, self.opmodes, m.pb.TIME,
                    domain=NonNegativeReals)
        m.lpb = Var(self.tasks, self.units, self.opmodes, m.pb.TIME,
                    domain=NonNegativeReals)
            ...
            for t in self.TIMEs:
                ...
                for i in self.I[j]:
                    for k in self.O[j]:
                        ...
                        costWear += self.D[i,j,k]*((1+self.eps)*m.usb[i,j,k,t]
                                                   - (1-self.eps)*m.lsb[i,j,k,t])
                        m.cons.add(m.usb[i,j,k,t] - m.lsb[i,j,k,t] >=
                                   m.sb.W[i,j,k,t])
            for t in self.TIMEp:
                ...
                for i in self.I[j]:
                    for k in self.O[j]:
                        ...
                        costWear += self.D[i,j,k]*((1+self.eps)*m.upb[i,j,k,t]
                                                   - (1-self.eps)*m.lpb[i,j,k,t])
                        m.cons.add(m.upb[i,j,k,t] - m.lpb[i,j,k,t] >=
                                   m.pb.N[i,j,k,t])
    ...
    def solve(self, solver='cplex',prefix=''):
        ...
        self.solver.options['timelimit'] = 60*60*2
        #self.solver.options['dettimelimit'] = 500000
        #self.solver.options['mipgap'] = 0.08
        self.solver.solve(self.model,
                          tee=True,
                          logfile="results/"+prefix+"STN.log",
                          keepfiles=True,
                          symbolic_solver_labels=True).write()
...
#        print(O)
    ...
    def trace(self, prefix=''):
        ...
        for t in TIMEs:
            ...
            print("\n    Unit residual lifetimes are:")
            for j in self.units:
                print("        {0:10s}  {1:6.1f} hrs".format(j,
                                                             model.sb.R[j,t]()))
